...
if( $mysqli->real_connect("localhost", "project", "project!", "project") )
{
	...
	{
		if( $_SESSION['inventory'][$i]['table_name'] == 'base_item' && $_SESSION['inventory'][$i]['no'] == $item )
		{
	...
	// 판매수 증가
	$sql = "UPDATE popular SET popular = popular + 1 WHERE table_name = 'base_item' AND no = '$no';";
	$mysqli->query($sql);
